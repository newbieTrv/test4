...
int main(void)
{
    ...
    /* Print start message */
    DebugPrintf("PSoC 6 With BLE Connectivity Multi-Slave\r\n");
    
    /* Bootloader */
    dfu_settings_t dfu_settings;
    ty_hal_flash_read_sector(APP_OTA_DFU_SETTINGS_ADDR, (uint8_t *)&dfu_settings, sizeof(dfu_settings_t));
    
    if (ty_app_is_valid(&dfu_settings)) {
        Cy_SysDisableCM4();
        
        DebugPrintf("/********************dfu settings******************/\r\n");
        DebugPrintf("update_start_address:    [%"PRIu32"] [0x%08"PRIX32"]\r\n", dfu_settings.write_offset, dfu_settings.update_start_address);
        DebugPrintf("bank bank_code      :    [%"PRIu32"]\r\n", dfu_settings.bank_1.bank_code);
        DebugPrintf("bank image_size     :    [%"PRIu32"]\r\n", dfu_settings.bank_1.image_size);
        DebugPrintf("bank image_crc      :    [0x%08"PRIX32"]\r\n", dfu_settings.bank_1.image_crc);
    
        /* firmware copy */
        DebugPrintf("image is valid, start copy\r\n");
        ty_bootloader_firmware_copy(dfu_settings.update_start_address, dfu_settings.bank_1.image_size); 

        memset((uint8_t *)&dfu_settings, 0x00, sizeof(dfu_settings_t));
        ty_hal_write_sector_with_erase(APP_OTA_DFU_SETTINGS_ADDR, (uint8_t *)&dfu_settings, sizeof(dfu_settings_t));
       
        DebugPrintf("image copy finish, system reset\r\n");
        NVIC_SystemReset();
    }
